...
int scoutSerials[4][2] = {{0x13A200,0x4151A85B},{0x13A200,0x4151A864},{0x13A200,0x4151A85C},{0x13A200,0x4151A848}}; 
...
//  unixTime = unixTime = 946702800 + 31536000*dateYear + 2678400*(dateMonth - 1) + 86400*(dateDay - 1) + 3600*dateHour + 60*dateMinute + dateSecond - 86400*2 - 3600*5;
  ...
  queenPayload[11] = time1;
  queenPayload[12] = time2; 
  queenPayload[13] = time3;
  queenPayload[14] = time4;
  ...
  count += 1;
  delay(300);                                          //Delay before querying scouts again
}
