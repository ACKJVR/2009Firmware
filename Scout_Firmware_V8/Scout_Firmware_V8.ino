...
void loop() {
  ...
  if(GPS.fix){
    ...
    latTXtest = (long) (latitude*1000000);
    lonTXtest = (long) (longitude*1000000);
    if(firstTime){
      lonTX = lonTXtest;
      latTX = latTXtest;
      firstTime = false;
      ...
      if(abs((latTX-latTXtest)<200)){
        latTX = latTXtest;
      }
      if(abs((lonTX-lonTXtest)<200)){
        lonTX = lonTXtest;
      }
    }
...
void markWaypoint(){
  if (millis() - last_interrupt > 1000){
    ...
    poiPayload[10] = scoutID;                    //Scout ID
